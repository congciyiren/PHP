...
class Page{
	...
	function pagedate($str1,$str2,$str3){
		...
		$this->numrows=$str2;
		$this->url =$str3;
		$this->page = intval($this->numrows/$this->pagesize);
		is($this->numrows%$this->pagesize){
			$this->pages ++;
		}
		$nPage = $_GET['page'];
	...
	function pageshow(){
		...
		else if($this->page >=$this->page-6){
			for($i=$this->page-9;$i<=$this->pages;$i++){
				if($i ==$this->page){
					echo "<span>".$i."</span>";
				}
				else{
