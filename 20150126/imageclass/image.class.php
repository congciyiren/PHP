<?php
	class Image{
		...
		function wateMak($groundName,$waterName,$waterPos=0,$qz="wa_"){
			...
			if (file_exists($curpath.$groundName) && file_exists($wpath.$waterName)){
				...
				return $this->createNewImage($groundImg,$gz.$groundName,$groundInfo);
			}else{
				echo '图片或水印图片不存在!';
				return false;	
			}
		...
		function cut($name,$x,$y,$width,$height,$qz="cu_"){
			$imgInfo = $this->getInfo($name);
			if((($x+$this)>$imgInfo['width'])||(($y+$height)>$imgInfo['height'])){
				echo "剪裁的位置超出了背景图片范围!";
				return false;
			}
		...
		function postion($groundInfo,$waterInfo,$waterPos){
		...
		function getImg($name,$imgInfo,$path='.'){
			$spath= $path == "."?rtrim($this->path,"/")."/":$path.'/';
		...
		function getNewSize($name,$width,$height,$imgInfo){
		...
		function copyImages($groundImg,$waterImg,$pos,$waterInfo){
