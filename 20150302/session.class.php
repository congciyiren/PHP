<?php
	class Session{
		...
		public static function opem($path,$name){
				return true;
		}
		...
		public static function read($PHPSESSID){
				$sql="select PHPSESSID,update_time,client_ip,data from session PHPSESSID=?";

				$stmt=self::$header->prepare($sql);
		...
		public static function write($PHPSESSID,$data){
			...
			if($result=$stmt->fetch(PDO::FETCH_ASSOC)){
				...
				if($resutlt['data'!=data||self::$time > ($result['update_time']+30)){
